...
public class RedisSubscriber implements MessageListener {
    ...
    private boolean sendChatMessage(RedisRecord record) {
        ...
        ChatMessageEmitEvent event = ChatMessageEmitEvent.of(record.isSystem(), record.childId(), record.body(),
            record.fileNames(), record.sentAt());
    ...
    private <T> boolean notify(EmitterType type, T key, EmitEventType event, Object payload) {
        if (!emitterManager.isExistConnection(type, key)) {
            return false;
        }
