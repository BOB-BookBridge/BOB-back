...
public class ManagementPostReportAdapter implements ManagementPostReportPort {
    ...
    @Override
    public Integer register(UUID managerId, UUID reportedId, Long postId) {
        String reason = "게시글 제재에 따른 신고 등록";

        var command = new RegisterReportByAdminCommand(
            ReportTarget.POST,
        ...
        return reportRegister.registerByManager(command);
    }
